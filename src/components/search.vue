  ...
  <div class="list_view">
    ...
    <div class="top_view" ref="topView">
      ...
      >
        <el-row :gutter="5" class="elRow">
          <div v-for="(t, i) in formArr" :key="i">
            <el-col
              v-if="t.DATALENGTH >= 100 && t.DEFAULT_TYPE == 'S'"
              :span="24"
            >
              <el-form-item
                :required="t.MANDATORY_FLAG == 'T'"
                :label="t.TEXT_PROMPT"
                v-if="
                  t.DEFAULT_TYPE != 'F' &&
                  t.DEFAULT_TYPE != 'D' &&
      ...
      <div class="btn_view" style="margin-left: 10px;margin-top: 5px;">
        ...
        <el-button size="mini" v-if="ADDshow" @click="aDD" type="primary"
          >新增</el-button
        >

        <el-button size="mini" v-if="REMOVEshow" @click="rEMOVE" type="danger"
          >删除</el-button
        >

        <el-button v-if="inquireTaps" size="mini" @click="inquireTap"
          >刷新</el-button
        >
        <el-button v-if="ExcelTaps" class="Excel" size="mini" @click="ExcelTap"
          >Excel导出</el-button
        >
      </div>
    </div>
  ...
  methods: {
    ...
    inquireTap() {
      ...
      this.formArr.forEach((itm) => {
        if (itm.DEFAULT_VALUE instanceof Array) {
         
          if (itm.DEFAULT_VALUE.length > 1) {
            let value = "";
            itm.DEFAULT_VALUE.forEach((item, index) => {
              if (index == itm.DEFAULT_VALUE.length - 1) {
                value += "'" + item;
              } else {
                if(index==0){
                    value += item + "'" + ",";
            ...
            Filter[itm.FIELD_NAME] = value;
          }else{
            Filter[itm.FIELD_NAME] = itm.DEFAULT_VALUE.join(',');
          }
