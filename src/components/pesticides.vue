<template>
    <div class="index">
        ...
        <div class="body1">
            <div class="body_top">
                <div class="title">农药检出频次分析地图</div>
                <!-- <div class="fold_right" @click="open_fold">
        <span>{{open_folds?'折叠':'展开'}}</span>
        <i :class="open_folds?'el-icon-arrow-up':'el-icon-arrow-down'"></i>
      </div> -->
        ...
        <div class="body1">
            <div class="body_top">
                <div class="title">农药检出频次分析统计</div>
            </div>
            <div class="body_bottom histogram_body_bottom">
                <el-col :span="24">
                    <!-- <div v-if="zhushow" class="histogram" id="histogram" :style="histogram"></div> -->
                    <query class="tables" ref="queryTable" height="100%" :totalshow="true" :listQuery="listQuery">
                    </query>
                </el-col>
            </div>
        </div>
        <div class="body1">
            ...
            <div class="body_bottom histogram_body_bottom">
                <el-col :span="24">
                    <!-- <div v-if="zhushow" class="histogram" id="histogram" :style="histogram"></div> -->
                    <query class="tables" ref="queryTable2" height="100%" :totalshow="true" :listQuery="listQuery2">
                    </query>
                </el-col>
            </div>
        </div>
        ...
        <el-dialog title="品质数据分析" fullscreen :visible.sync="dialogVisible" @close="dialogVisibleClose">
            <QualityTwoChildren ref="QualityTwoChildren" name="品质数据分析" :UserId="UserId" :CITY="CITY">
            </QualityTwoChildren>
        </el-dialog>
    </div>
</template>
            ...
            this.$getReq("/ashx/Common.ashx", "post", data).then((res) => {
                ...
                if (data2.length > 0) {
                    ...
                    nameList.map((r) => {
                        data2.map((res) => {
                            if (r.name == res.CITY) {
                                r.val.push({ name: res.XMMC, value: res.GS, city: r.name });
                            }
                        });
                    });
                    nameList.map((r) => {
